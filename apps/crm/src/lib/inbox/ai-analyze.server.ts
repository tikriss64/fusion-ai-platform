...
export interface AnalysisResult {
  id: string;
  type: "Cliente" | "Proveedor" | "Reclamación" | "Comercial" | "Urgente" | "Info" | "Ruido";
  summary: string;
  promise: string | null;
...
// 3.5-flash = principal (actual, junio 2026); 3.1-flash-lite = fallback de bajo coste.
...
- "type": uno de "Cliente","Proveedor","Reclamación","Comercial","Urgente","Info","Ruido"
  REGLA CLAVE para distinguir:
  · "Cliente": una persona que te escribe directamente sobre un servicio (vaciado, limpieza), un presupuesto, una visita, etc.
  · "Proveedor": un proveedor o colaborador real con quien trabajas (facturas suyas, materiales, contenedores...).
  · "Comercial": una OPORTUNIDAD DE NEGOCIO REAL — alguien interesado en contratar tu servicio, una empresa que quiere colaborar de verdad.
  · "Ruido": newsletters, boletines, promociones, marketing masivo, ofertas genéricas, notificaciones automáticas de plataformas (ej.: Booking, Yoojo, Club Leroy Merlin, idealista/news, Ahrefs, Google Search Console, ofertas de publicidad). NO es spam, pero NO requiere ninguna acción y NO es de un cliente ni proveedor tuyo. Ante la duda entre "Comercial" e "Info" para un correo masivo/automático, usa "Ruido".
